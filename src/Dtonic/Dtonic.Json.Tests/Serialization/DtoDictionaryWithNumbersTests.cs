...
public class DtoDictionaryWithNumbersTests
{
    ...
    {
        //Arrange
        var dtoDictionary = new DtoDictionaryWithNumbers(null);
    ...
    {
        //Arrange
        var dict = new Dictionary<string, decimal?>() { { "one", 1.1m }, { "two", 2 } };
        var dtoDictionary = new DtoDictionaryWithNumbers(dict);

        //Act
        var s = dtoDictionary.StringifyWithKey();

        //Assert
        Assert.AreEqual("\"dtoDictionary\":[{\"one\":1.1},{\"two\":2}]", s);
    }
    ...
    {
        //Arrange
        var dict = new Dictionary<string, decimal?>() { { "one", 1 }, { "two", null } };
        var dtoDictionary = new DtoDictionaryWithNumbers(dict);
